import React, { useState, useEffect, useRef } from "react";
import { useNavigate } from "react-router-dom";
import refresh from "../api/refresh";
import MemberItem from "./MemberItem";

function ViewMembersModal({ setModalOpen, clubId }: PropsType) {
    const navigate = useNavigate();

    const [members, setMembers] = useState([]);
    const [max, setMax] = useState("");
    const [num, setNum] = useState("");

    const closeModal = () => {
        setModalOpen(false);
    }

    useEffect(() => {
        getClubInfo();
    }, [])

    const getClubInfo = async () => {
        await refresh.get(`/clubs/${clubId}`)
            .then((res) => {
                console.log(res);
                setMembers(res.data.response.members);
                setMax(res.data.response.max);
                setNum(res.data.response.num);
            })
            .catch((err) => {
                console.log(err);
            })
    }

    return (
        <div id="ViewMembersModal" class="overflow-y-auto overflow-x-hidden justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full h-full md:h-auto">
                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            Members
                        </h3>
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="makeGroupModal" onClick={() => {
                            closeModal()
                        }}>
                            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                            <span class="sr-only">Close modal</span>
                        </button>
                    </div>
                    <div className="text-xl font-medium text-black text-center">{num} / {max}</div>
                    <div class = "flex-col items-start justify-center m-1">
            {members.map((member, index) => {
                return (
                    <MemberItem
                        key = {index}
                        member = {member}
                        />
                );
            })}
        </div>
                </div>
                
            </div>
        </div>
    )
}

export default ViewMembersModal;

